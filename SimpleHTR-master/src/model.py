...
class Model:
    ...
    def setup_ctc(self) -> None:
        ...
        # word beam search decoding (see https://github.com/githubharald/CTCWordBeamSearch)
            ...
            word_chars = open( os.path.dirname(os.path.abspath('charList.txt')).replace("\\", "/")[2:] +'/SimpleHTR-master/model/charList.txt').read().splitlines()[0]
            corpus = open(os.path.dirname(os.path.abspath('corpus.txt')).replace("\\", "/")[2:] +'/SimpleHTR-master/data/corpus.txt').read()
